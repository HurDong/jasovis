...
function resume(id = 55, numbered = false) {
  return `<!doctype html><html><body><textarea class="answer"></textarea><button onclick="window.saves++">저장하기</button><script>
  window.saves=0; window.model={resume:{id:${id},title:'예시기업 ICT ${id}'},qnas:${JSON.stringify(questions.map((question, i) => ({ id: 91 + i, number: i + 1, question: (numbered ? (i + 1) + '. ' : '') + (process.env.JSL_GPT_DETAILED === '1' && detailedPairs[i] ? detailedPairs[i].source : question), answer: '기존 ' + (i + 1) })))},
...
module.exports = { questions, answers, block, full, turn, chat, resume, detailedPairs };
