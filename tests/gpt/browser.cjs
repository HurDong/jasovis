...
(async () => {
  ...
  try {
    await context.route('https://jasoseol.com/**', route => route.fulfill({ contentType: 'text/html; charset=utf-8', body: F.resume(Number(new URL(route.request().url()).pathname.split('/')[2]) || 55, process.env.JSL_GPT_NUMBERED === '1') }));
    ...
    await gpt.getByRole('button', { name: '자소설에서 확인 ↗', exact: true }).first().click();
    await gpt.waitForFunction(() => !document.querySelector('[data-jsl-gpt] button').disabled);
    assert.equal(await target.evaluate(() => model.currentQnaIndex), 3);
    assert.equal(await target.locator('textarea.answer').inputValue(), '네 번째 수정\n');
    await replace(F.block(2, { answer: '두 번째 수정\n' }));
    await apply(); await waitStatus('문항 2 입력 확인');
    await gpt.getByRole('button', { name: '자소설에서 확인 ↗', exact: true }).first().click();
    await gpt.waitForFunction(() => !document.querySelector('[data-jsl-gpt] button').disabled);
    assert.equal(await target.evaluate(() => model.currentQnaIndex), 1);
    assert.equal(await target.locator('textarea.answer').inputValue(), '두 번째 수정\n');
    assert.equal(await target.evaluate(() => saves), 0);
    console.log('PASS: review opens first applied question and single question 2 without another write or save');
    await target.evaluate(() => { model.currentQnaIndex = 0; model.switch_qna = function () {}; });
    await gpt.getByRole('button', { name: '자소설에서 확인 ↗', exact: true }).first().click();
    await waitStatus('해당 문항으로 이동하지 못했습니다');
    assert.equal(await target.evaluate(() => model.currentQnaIndex), 0);
    assert.equal((await answers())[1], '두 번째 수정\n');
    await target.evaluate(() => { model.switch_qna = function (number) { this.currentQnaIndex = number - 1; document.querySelector('textarea').value = this.qnas[number - 1].answer; }; });
    const originalQuestion = await target.evaluate(() => model.qnas[1].question);
    await target.evaluate(() => { model.qnas[1].question = '변경된 가상 문항'; });
    await gpt.getByRole('button', { name: '자소설에서 확인 ↗', exact: true }).first().click();
    await waitStatus('확인할 문항이 변경되었습니다');
    assert.equal(await target.evaluate(() => model.currentQnaIndex), 0);
    await target.evaluate(question => { model.qnas[1].question = question; }, originalQuestion);
    console.log('PASS: failed switch and changed question report errors without modifying answers');
    await gpt.reload();
    await gpt.locator('[data-jsl-gpt]').getByText('예시기업 ICT 55', { exact: true }).waitFor();
