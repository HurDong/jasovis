...
분석기 v2는 `spacingDifference`로 공백 경계 삽입·삭제를 비교하고 `restatedGuidance`로 뒤 안내를 분류한다. UI/worker에서 비교 규칙을 재구현하지 않는다. `gpt:prepare`가 선택된 쌍의 `comparisons`를 반환하며 UI는 안내 원문을 접어서 제공한다. 입력 직전 검증·원문 지문에는 완화 키를 사용하지 않는다. v1 대응 기억은 기존 버전 필터에서 폐기하며 새 규칙으로 재학습하지 않는다.
